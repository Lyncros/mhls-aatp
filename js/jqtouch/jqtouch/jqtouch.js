    ...
    $.jQTouch = function(options) {
        ...
        function init(options) {
            ...
            $(function(){
				...
				document.styleSheets[0].insertRule((touchSelectors.join(',')+"{-webkit-touch-callout:none}", 0);
        ...
        function goForward(to) {
            // Init the param
            if (forward.length > 1) {
                var numberOfPages = Math.min(parseInt(to || 1, 10), forward.length-1);

                // Search through the history for an ID
                if( isNaN(numberOfPages) && typeof(to) === "string" && to != '#' ) {
                    for( var i=1, length=forward.length; i < length; i++ ) {
                        if( '#' + forward[i].id === to ) {
                            numberOfPages = i;
                            break;
                        }
                ...
                var animation = forward[0].animation;
                var fromPage = forward[0].page;

                // Remove all pages in front of the target page
                hist.unshift.apply(hist, forward.splice(0, numberOfPages).reverse());

                // Grab the target page
                var toPage = forward[0].page;
                ...
                console.error('No pages in forward.');
                return false;
            }
        }
        ...
        function goBack(to) {
            ...
            if (hist.length > 1) {
                ...
                // Remove all pages in front of the target page
                forward.unshift.apply(forward, hist.splice(0, numberOfPages).reverse());
        ...
        function dumbLoopStart() {
            dumbLoop = setInterval(function(){
				...
				var curid = currentPage.attr('id'), l, h = location.hash;
                if (h == '') {
                    location.hash = '#' + curid;
                } else if (h != '#' + curid) {
                    l = forward.length;
					console.log("forward.length = " + l);
					while (l)
					{
						if ('#' + forward[--l].id == h)
						{
        ...
        function insertPages(nodes, animation) {
            ...
            $(nodes).each(function(index, node){
                ...
                $.event.trigger('jqtouch.page-inserted', {node: $node.appendTo($body)});
