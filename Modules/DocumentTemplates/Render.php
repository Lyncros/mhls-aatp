<?
	...
	$Search = new CSearch("DocumentTemplates");
	...
	if(CSecurity::CanAccess($this->Parent->Name, "AddEdit")) {
		$Search->AddColumn("<div class='Icon_Edit' style='display: none;'></div>", "<div class='Icon_Edit' onClick=\"MDocumentTemplates.Window_AddEdit([[ID]]);\"></div>", "22px");
	}

	if(CSecurity::CanAccess($this->Parent->Name, "Delete")) {
		$Search->AddColumn("<div class='Icon_Delete' style='display: none;'></div>", "<div class='Icon_Delete' onClick=\"MDocumentTemplates.Window_Delete([[ID]]);\"></div>", "22px");
	}

	$Search->SetDefaultColumn(1);

	$Search->OnInit();

	$Search->OnRenderTitle("Document Templates", Array("Export" => true, "MaxView" => true));
	$Search->OnRender();
